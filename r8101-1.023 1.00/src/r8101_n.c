...
static const struct {
	...
	_R("RTL8105E", MCFG_8105E_1, 0xff7e1880),
...
u32 mdio_read(void __iomem *ioaddr, u32 RegAddr)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (ReadMMIO32(PHYAR) & PHYAR_Flag) {
			value = (int) (ReadMMIO32(PHYAR) & PHYAR_Data_Mask);
			IODelay(20);
			break;
		}
	}
...
rtl8101_xmii_link_ok(struct net_device *dev)
{
	...
	return ReadMMIO8(PHYstatus) & LinkStatus;
}
			  ...
			  void __iomem *ioaddr)
{
	...
	if (tp->link_ok(dev)) {
		...
		if (mcfg == MCFG_8105E_2 || mcfg == MCFG_8105E_3 ||
		    mcfg == MCFG_8105E_4) {
			...
			if (ReadMMIO8(PHYstatus) & _10bps) {
				...
				WriteERI(0x1B0, 2, 0, ERIAR_ExGMAC);
				spin_lock_irqsave(&tp->phy_lock, flags);
				mdio_write( ioaddr, 0x1F, 0x0004);
				data32 = mdio_read( ioaddr, 0x10);
				data32 |= 0x0400;
				data32 &= ~0x0800;
		...
		} else if (mcfg == CFG_METHOD_14 || mcfg == CFG_METHOD_15 || 
					mcfg == CFG_METHOD_16) {
			if (ReadMMIO8(PHYstatus) & _10bps) {
				WriteERI(0x1D0, 2, 0x4d02, ERIAR_ExGMAC);
				WriteERI(0x1DC, 2, 0x0060, ERIAR_ExGMAC);
...
rtl8101_powerdown_pll(struct net_device *dev)
{
	...
	if ((mcfg == MCFG_8105E_2 || mcfg == MCFG_8105E_3 ||
	     mcfg == MCFG_8105E_4) && (eee_enable == 1))
		rtl8101_disable_EEE(tp);
...
rtl8101_hw_phy_config(struct net_device *dev)
{
	...
	if (mcfg == CFG_METHOD_4) {
		...
		WriteGMII16(0x1F, 0x0000);
	} else if (mcfg == MCFG_8105E_2 || mcfg == MCFG_8105E_3 ||
		   mcfg == MCFG_8105E_4) {
		WriteGMII16(0x1F, 0x0000);
		WriteGMII16(0x18, 0x0310);
		WriteGMII16(0x1F, 0x0000);
		      ...
		      void __iomem *ioaddr)
{
	...
	rtl8101_rar_set(tp, dev->perm_addr);
#else
...
rtl8101_aspm_fix1(struct net_device *dev)
{
	...
	data = ReadCSI32(0x110);

	if ((data & (1 << 7)) && (data & (1 << 8))) {
		WriteEPHY16(0x01, 0x2e65);
		WriteEPHY16(0x01, 0x6e65);
	}
}
...
rtl8101_aspm_fix2(struct net_device *dev)
{
	...
	if (tp->link_ok(dev))
		WriteCSI32(0x70c, csi_tmp | 0x3F000000);
	else
...
rtl8101_check_eeprom(struct rtl8101_private *tp)
{
	...
	WriteMMIO8(Cfg9346, Cfg9346_Unlock);
	WriteMMIO8(Config5, ReadMMIO8(Config5) & ~UWF);
	WriteMMIO8(Cfg9346, Cfg9346_Lock);

	rtl_eeprom_write_sc(ioaddr, 0x00, 0x8129);

	WriteMMIO8(Cfg9346, Cfg9346_EEM0);
	mdelay(15);
...
#else
	memcpy(tp->org_mac_addr, dev->dev_addr, dev->addr_len);
#endif
...
static int rtl8101_open(struct net_device *dev)
{
	...
	WriteMMIO32(CounterAddrHigh, (u64)tp->paddr >> 32);
	WriteMMIO32(CounterAddrLow, (u64)tp->paddr & (DMA_BIT_MASK(32) | CounterDump | BIT_0));
...
//		WriteMMIO8(Config1, 0xDF);
	...
	} else if (mcfg == CFG_METHOD_15 || mcfg == CFG_METHOD_16) {
		u8	pci_config;
	...
	WriteMMIO16(CPlusCmd, tp->cp_cmd);
#else
	rtl8101_set_features(dev, dev->features);
#endif
...
#endif
{
	...
	do {
		...
		if (status & rtl8101_napi_event) {
			WriteMMIO16(IntrMask, rtl8101_intr_mask & ~rtl8101_napi_event);
...
//	WriteMMIO32(RxMissed, 0);
	 ...
	 */
	if (ReadMMIO16(IntrMask)) {
		if ((u16)ReadCSI32(0x10) != 0xffff)
			goto core_down;
	}

	if (mcfg == CFG_METHOD_8) {
		WriteEPHY16(0x19, ReadEPHY16(0x19)&~0x0040);
		WriteMMIO8(0xF4, ReadMMIO8(0xF4) & ~BIT_0);
	} else if (mcfg == MCFG_8105E_1) {
		WriteEPHY16(0x19, 0xFF64);
	}
