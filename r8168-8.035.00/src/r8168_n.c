	   ...
	   u32 value)
{
	...
	} else if (mcfg == CFG_METHOD_21 || mcfg == CFG_METHOD_22 || mcfg == CFG_METHOD_23 || mcfg == CFG_METHOD_24) {
		u32 data32;
		u16 ocp_addr;
...
rtl8168_xmii_link_ok(struct net_device *dev)
{
	...
	retval = ReadMMIO8(PHYstatus) & LinkStatus;
...
rtl8168dp_10mbps_gphy_para(struct net_device *dev)
{
	...
	spin_lock_irqsave(&tp->phy_lock, flags);
	if ((status & LinkStatus) && (status & _10Mbps)) {
		WriteGMII16(0x1f, 0x0000);
			  ...
			  void __iomem *ioaddr)
{
	...
	if (tp->link_ok(dev)) {
		if (mcfg == MCFG_8168F_1 || mcfg == MCFG_8168F_2 || mcfg == MCFG_8411_1) {
			...
			if (netif_running(dev) && (ReadMMIO8(ChipCmd) & (CmdRxEnb | CmdTxEnb))==0) {
				int timeout;
				for (timeout = 0; timeout < 10; timeout++) {
		...
		} else if ((mcfg == CFG_METHOD_21 || mcfg == CFG_METHOD_22 || mcfg == CFG_METHOD_24) && netif_running(dev)) {
			if ((ReadMMIO8(ChipCmd) & (CmdRxEnb | CmdTxEnb)) == 0) {
				rtl8168_init_ring_indexes(tp);
...
rtl8168_powerdown_pll(struct net_device *dev)
{
	...
	if ((mcfg == MCFG_8168DP_1 || mcfg == MCFG_8168DP_2 ||
	     mcfg == MCFG_8168DP_3 || mcfg == CFG_METHOD_23) && rtl8168_check_dash(tp)) {
	...
	if (tp->wol_enabled == WOL_ENABLED) {
		spin_lock_irqsave(&tp->phy_lock, flags);
		WriteGMII16(0x1F, 0x0000);
		auto_nego = ReadGMII16(MII_ADVERTISE);
		...
		WriteGMII16(MII_ADVERTISE, auto_nego);
...
#define WAKE_ANY (WAKE_PHY | WAKE_MAGIC | WAKE_UCAST | WAKE_BCAST | WAKE_MCAST)
	...
	case CFG_METHOD_22:
	case CFG_METHOD_24:
		csi_tmp = ReadERI(0xDE, 4, ERIAR_ExGMAC);
		if (csi_tmp & BIT_0)
		...
		struct ethtool_wolinfo *wol)
{
	...
	case CFG_METHOD_24:		
		...
		if (wol->wolopts & WAKE_MAGIC)
			...
			csi_tmp &=~ BIT_0;
		WriteERI(0xDE, 4, csi_tmp, ERIAR_ExGMAC);
		break;
	default:
		tmp = ARRAY_SIZE(cfg);
		break;
		    ...
		    u32 data)
{
	...
	WriteMMIO16(CPlusCmd, tp->cp_cmd);
		  ...
		  struct ethtool_cmd *cmd)
{
	...
	else if (status & _100Mbps)
		cmd->speed = SPEED_100;
	else if (status & _10Mbps)
		cmd->speed = SPEED_10;
	...
	cmd->duplex = ((status & _1000Mbps) || (status & FullDup)) ?
			  ...
			  u64 *data)
{
	...
	WriteMMIO32(CounterAddrLow, 0);
	WriteMMIO32(CounterAddrHigh, 0);
...
static int rtl8168_enable_EEE(struct rtl8168_private *tp)
{
	...
	case MCFG_8168E_VL_2:
		spin_lock_irqsave(&tp->phy_lock,flags);
		data = ReadERI(0x1B0 ,4,ERIAR_ExGMAC) | 0x0003;
...
static void
rtl8168_hw_phy_config(struct net_device *dev)  //RTL8168HwPhyConfig
{
	struct rtl8168_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned long flags;
    ...
    if (mcfg == MCFG_8168F_1) {
		...
		WriteMMIO8(0xD0, ReadMMIO8(0xD0) | BIT_6 | BIT_7);
		WriteMMIO8(0xF2, ReadMMIO8(0xF2) | BIT_6);
		WriteERI(0x5F0, 2, 0x4f87, ERIAR_ExGMAC);
		gphy_val = ReadERI(0xD4, 2, ERIAR_ExGMAC);
		gphy_val |= (0x3F << 7);
		WriteERI(0xD4, 2, gphy_val, ERIAR_ExGMAC);
		gphy_val = ReadERI(0x1D0, 1, ERIAR_ExGMAC);
		gphy_val |= BIT_1;
		WriteERI(0x1D0, 1, gphy_val, ERIAR_ExGMAC);
		...
		csi_tmp |= BIT_0 | BIT_1;
		WriteERI(0x3F2, 2, csi_tmp, ERIAR_ExGMAC);
		WriteERI(0x1E2, 2, 0x04, ERIAR_ExGMAC);
		   ...
		   void __iomem **ioaddr_out)
{
	...
	if (pm_cap) {
		...
		pciDev->configRead16(pm_cap + PCI_PM_CTRL, &pwr_command);
	} else {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
		if (netif_msg_probe(tp)) {
...
rtl8168_init_sequence(struct rtl8168_private *tp)
{
	...
	case CFG_METHOD_23:
	case CFG_METHOD_24:
		WriteMMIO32(RxConfig, 0xcf00);
		WriteMMIO8(0xF2, ReadMMIO8(0xF2) | BIT_3);
		for (i = 0; i < 10; i++) {
			IODelay(100);
			if (ReadMMIO32(TxConfig) & BIT_11) {
				if ((ReadMMIO8(MCUCmd_reg) & Txfifo_empty) &&
				    (ReadMMIO8(MCUCmd_reg) & Rxfifo_empty))
					break;
			}
		}
		WriteMMIO8(ChipCmd, ReadMMIO8(ChipCmd) & ~(CmdRxEnb | CmdTxEnb));
		mdelay(1);
...
rtl8168_esd_timer(unsigned long __opaque)
{
	...
	pci_read_config_byte(pdev, kIOPCIConfigCommand, &cmd);
	if (cmd != tp->pci_cfg_space.cmd) {
		pci_write_config_byte(pdev, kIOPCIConfigCommand, tp->pci_cfg_space.cmd);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_0, &io_base_l);
	if (io_base_l != tp->pci_cfg_space.io_base_l) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_0, tp->pci_cfg_space.io_base_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_2, &mem_base_l);
	if (mem_base_l != tp->pci_cfg_space.mem_base_l) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_2, tp->pci_cfg_space.mem_base_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_2 + 2, &mem_base_h);
	if (mem_base_h!= tp->pci_cfg_space.mem_base_h) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_2 + 2, tp->pci_cfg_space.mem_base_h);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_3, &resv_0x1c_l);
	if (resv_0x1c_l != tp->pci_cfg_space.resv_0x1c_l) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_3, tp->pci_cfg_space.resv_0x1c_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_3 + 2, &resv_0x1c_h);
	if (resv_0x1c_h != tp->pci_cfg_space.resv_0x1c_h) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_3 + 2, tp->pci_cfg_space.resv_0x1c_h);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_4, &resv_0x20_l);
	if (resv_0x20_l != tp->pci_cfg_space.resv_0x20_l) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_4, tp->pci_cfg_space.resv_0x20_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_4 + 2, &resv_0x20_h);
	if (resv_0x20_h != tp->pci_cfg_space.resv_0x20_h) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_4 + 2, tp->pci_cfg_space.resv_0x20_h);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_5, &resv_0x24_l);
	if (resv_0x24_l != tp->pci_cfg_space.resv_0x24_l) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_5, tp->pci_cfg_space.resv_0x24_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_BASE_ADDRESS_5 + 2, &resv_0x24_h);
	if (resv_0x24_h != tp->pci_cfg_space.resv_0x24_h) {
		pciDev->configWrite16(PCI_BASE_ADDRESS_5 + 2, tp->pci_cfg_space.resv_0x24_h);
		tp->esd_flag = 1;
	}
	...
	pciDev->configRead16(PCI_SUBSYSTEM_VENDOR_ID, &resv_0x2c_l);
	if (resv_0x2c_l != tp->pci_cfg_space.resv_0x2c_l) {
		pciDev->configWrite16(PCI_SUBSYSTEM_VENDOR_ID, tp->pci_cfg_space.resv_0x2c_l);
		tp->esd_flag = 1;
	}

	pciDev->configRead16(PCI_SUBSYSTEM_VENDOR_ID + 2, &resv_0x2c_h);
	if (resv_0x2c_h != tp->pci_cfg_space.resv_0x2c_h) {
		pciDev->configWrite16(PCI_SUBSYSTEM_VENDOR_ID + 2, tp->pci_cfg_space.resv_0x2c_h);
		tp->esd_flag = 1;
	}
	
	pci_nvidia_geforce_6200 = ReadCSI32(PCI_DEVICE_ID_NVIDIA_GEFORCE_GO_6200);
	...
	pci_nvidia_geforce__6250_1 = ReadCSI32(PCI_DEVICE_ID_NVIDIA_GEFORCE_GO_6250_1);
		 ...
		 const struct pci_device_id *ent)
{
	...
	if ((mcfg == MCFG_8168D_1) || (mcfg == MCFG_8168D_2))
	...
	if (tp->eeprom_type != EEPROM_TYPE_NONE) {
		...
		WriteMMIO32(MAC4, (mac_addr[3] << 16) | mac_addr[2]);
		WriteMMIO8(Cfg9346, Cfg9346_Lock);
	}
...
rtl8168_hw_start(struct net_device *dev)
{
	...
	WriteMMIO8(Config5, ReadMMIO8(Config5) & ~BIT_7);
...
//-----------------      
		...
		ephy_data = ReadEPHY16(0x0B);
		WriteEPHY16(0x0B, ephy_data|0x48);
		ephy_data = ReadEPHY16(0x19);
		ephy_data &= ~0x20;
		WriteEPHY16(0x19, ephy_data|0x50);
		ephy_data = ReadEPHY16(0x0C);
		ephy_data &= ~0x100;
		WriteEPHY16(0x0C, ephy_data|0x20);
...
//		WriteMMIO8(0xF2, (ReadMMIO8(0xF2) | BIT_2 | BIT_0) & ~BIT_1);	// early tally counter causes kernel panic
		...
		WriteERI(0xCC, 4, 0x00000050, ERIAR_ExGMAC);
		WriteERI(0xd0, 4, 0x00000060, ERIAR_ExGMAC);
	} else if (mcfg == CFG_METHOD_21 || mcfg == CFG_METHOD_22 || mcfg == CFG_METHOD_24) {
		WriteERI(0xC8, 1, 0x02, ERIAR_ExGMAC);
		WriteERI(0xCA, 1, 0x08, ERIAR_ExGMAC);
		...
		WriteEPHY16(0x00, 0x10a3);
		WriteEPHY16(0x06, 0xf030);
		WriteEPHY16(0x08, 0x2006);
		WriteEPHY16(0x0d, 0x1666);
		...
		csi_tmp = ReadERI(0xd4, 2, ERIAR_ExGMAC);
		csi_tmp |= 0x1f80;
		WriteERI(0xd4, 2, csi_tmp, ERIAR_ExGMAC);
	...
	if (tp->rx_fifo_overflow == 0) {
		...
		WriteMMIO16(IntrMask, rtl8168_intr_mask);
	...
	if (!tp->pci_cfg_is_read) {
		pci_read_config_byte(pdev, kIOPCIConfigCommand, &tp->pci_cfg_space.cmd);
		pciDev->configRead16(PCI_BASE_ADDRESS_0, &tp->pci_cfg_space.io_base_l);
		pciDev->configRead16(PCI_BASE_ADDRESS_0 + 2, &tp->pci_cfg_space.io_base_h);
		pciDev->configRead16(PCI_BASE_ADDRESS_2, &tp->pci_cfg_space.mem_base_l);
		pciDev->configRead16(PCI_BASE_ADDRESS_2 + 2, &tp->pci_cfg_space.mem_base_h);
		pciDev->configRead16(PCI_BASE_ADDRESS_3, &tp->pci_cfg_space.resv_0x1c_l);
		pciDev->configRead16(PCI_BASE_ADDRESS_3 + 2, &tp->pci_cfg_space.resv_0x1c_h);
		pci_read_config_byte(pdev, PCI_INTERRUPT_LINE, &tp->pci_cfg_space.ilr);
		pciDev->configRead16(PCI_BASE_ADDRESS_4, &tp->pci_cfg_space.resv_0x20_l);
		pciDev->configRead16(PCI_BASE_ADDRESS_4 + 2, &tp->pci_cfg_space.resv_0x20_h);
		pciDev->configRead16(PCI_BASE_ADDRESS_5, &tp->pci_cfg_space.resv_0x24_l);
		pciDev->configRead16(PCI_BASE_ADDRESS_5 + 2, &tp->pci_cfg_space.resv_0x24_h);
		pciDev->configRead16(PCI_SUBSYSTEM_VENDOR_ID, &tp->pci_cfg_space.resv_0x2c_l);
		pciDev->configRead16(PCI_SUBSYSTEM_VENDOR_ID + 2, &tp->pci_cfg_space.resv_0x2c_h);
		tp->pci_cfg_space.pci_nvidia_geforce_6200 = ReadCSI32(PCI_DEVICE_ID_NVIDIA_GEFORCE_GO_6200);
...
rtl8168_pcierr_interrupt(struct net_device *dev)
{
	...
	pciDev->configRead16(kIOPCIConfigCommand, &pci_cmd);
	pciDev->configRead16(kIOPCIConfigStatus, &pci_status);
	...
	pciDev->configWrite16(kIOPCIConfigCommand,
			      pci_cmd | kIOPCIConfigCommand_SERR | kIOPCIConfigCommand_PARITY);

	pciDev->configWrite16(kIOPCIConfigStatus,
		pci_status & (kIOPCIConfigStatus_DETECTED_PARITY |
		kIOPCIConfigStatus_SIG_SYSTEM_ERROR | kIOPCIConfigStatus_REC_MASTER_ABORT |
		kIOPCIConfigStatus_REC_TARGET_ABORT | kIOPCIConfigStatus_SIG_TARGET_ABORT));
...
#endif
{
	...
	do {
		...
		if (status & rtl8168_napi_event) {
			...
			WriteMMIO16(IntrMask, rtl8168_intr_mask & tp->intr_mask);
